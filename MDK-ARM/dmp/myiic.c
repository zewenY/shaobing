...
#if	1		
	#define SDA_PIN_IDX		14	//PIN_IDX X = GPIO_PIN_X
	#if defined (__STM32F4xx_HAL_H)
		#define SDA_IN()	\
											{	SDA_PORT->MODER&=~(3<<(SDA_PIN_IDX*2));\
												SDA_PORT->MODER|=(0<<(SDA_PIN_IDX*2));}	//as input 
		
		#define SDA_OUT()	\
											{	SDA_PORT->MODER&=~(3<<(SDA_PIN_IDX*2));\
												SDA_PORT->MODER|=(1<<(SDA_PIN_IDX*2));}	//as output
	#else 	
		/*stm32 F1 series	only!!!*/
		//only if SDA = gpio_pin_4 can use this!!!
		#define SDA_IN()  {SDA_PORT->CRL&=0XFFF0FFFF;SDA_PORT->CRL|=8<<16;}		
		#define SDA_OUT() {SDA_PORT->CRL&=0XFFF0FFFF;SDA_PORT->CRL|=3<<16;}
	#endif
#else
		//will slower than up!!!
...
u8 IIC_Read_Bytes(u8 devAddr, u8 reg,  u8 *buff, u8 len){
	...
	IIC_Wait_Ack();
	while(len){
		if(len == 1)*buff = IIC_Read_Byte(0);	//send nACK
